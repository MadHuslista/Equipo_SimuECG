...
#Creación de la Ventana y su título
fig_gen = plt.figure(figsize=[5,2])
fig_gen.suptitle("Control de Parámetros Generales", y = 0.9)

#Se crean las barras con add_axes([left, bottom, width, heigth], color). 
#Sobre estos elementos se construye los slider y los botones de reset y simulación
...
#Se toman las barras creadas y se convierten en sliders y botones para reset y simulación 
s_hrmean    = Slider(ax_hrmean, 'Frecuencia Cardíaca Promedio', 20, 200, valinit=param_gener[0], valstep=1, valfmt = "%3d [Hz]")
s_resp      = Slider(ax_Resp_by_min, 'Frecuencia Respiratoria Promedio', 0, 70, valinit=param_gener[1], valstep=1, valfmt = "%2d  [Hz]")
s_Amp_ECG   = Slider(ax_Amp_ECG, 'Amplitud Máxima ECG', 0, 5, valinit=param_gener[2], valstep=0.1 ,valfmt = "%2.1f [mV]")
s_n         = Slider(ax_n, 'Pulsaciones Simuladas', 2, 100, valinit=param_gener[3], valstep=1,valfmt = "%3d")
s_dt        = Slider(ax_dt, 'Frecuencia de Muestreo 10^x',1,4, valinit=(-np.log10(param_gener[4])), valstep=1,valfmt = "%1d") #Calcula el exponente positivo correspondiente al dt
s_FPS       = Slider(ax_FPS, 'Cuadros por Segundo', 25, 50, valinit=param_gener[5], valstep=5, valfmt = "%2d  [FPS]")
...
#Se define la función que será llamada cuando se aprieta el botón reset
#En ésta se llama al método de los sliders para resetear sus valores a su valor default. 
def reset_gen(event):
    s_hrmean.reset()
    s_resp.reset()
    s_Amp_ECG.reset()
...
#Se establece el vínculo entre la acción de apretar el botón y la función llamada.    
...
#Se toman las barras creadas y se convierten en sliders y botones para reset y simulación 
s_Anoise    = Slider(ax_Anoise, 'Amplitud Ruido Aleatorio', 0, 1, valinit=param_Artf[0], valstep=0.01, valfmt = "%4.2f [mV]")
s_Hznoise   = Slider(ax_Hz_noise, 'Frecuencia Interferencia', 0, 100, valinit=param_Artf[1], valstep=1, valfmt = "%3d  [Hz]")
s_AHznoise  = Slider(ax_AHznoise, 'Amplitud Interferencia', 0, 1, valinit=param_Artf[2], valstep=0.01, valfmt = "%4.2f [mV]")
...
####################### 0.- PARÁMETROS HVR ####################################
"""
#Ventana para control de Parámetros HVR (Heart Variability Rate)
...
#Se toman las barras creadas y se convierten en sliders y botones para reset y simulación 
s_hrstd     = Slider(ax_hrstd, 'Desviación Estándar F. Cardíaca', 0, 10, valinit=param_HVR[0], valstep=0.1, valfmt = "%4.1f [Hz]")
s_c1        = Slider(ax_c1, 'Desviación Estándar Onda Mayer', 0, 0.5, valinit=param_HVR[1], valstep=0.01, valfmt = "%4.2f [Hz]")
s_c2        = Slider(ax_c2, 'Desviación Estándar Onda RSA', 0, 0.5, valinit=param_HVR[2], valstep=0.01, valfmt = "%4.2f [Hz]")
s_f1        = Slider(ax_f1, 'Frecuencia Central Onda Mayer', 0, 0.5, valinit=param_HVR[3], valstep=0.01, valfmt = "%4.2f [Hz]")
s_f2        = Slider(ax_f2, 'Frecuencia Central Onda RSA', 0, 0.5, valinit=param_HVR[4], valstep=0.01, valfmt = "%4.2f [Hz]")
...
#Creación del plot circular para la visualización de ángulos. 
ax_circle = plt.subplot(frame_on=False)                 #define el subplot para el círculo (en la práctica el sector para este)
plt.axis('off')                                         #permite ocultar los ejes X, Y                
circle = plt.Circle((0,0), radius = 1, fill = False)    #crea el dibujo del círculo base. 
ax_circle.add_patch(circle)                             #se vincula este dibujo dentro de un subplot (el creado previamente)
ax_circle.axis('scaled')                                #Para que no se vea defrmado
ax_circle.set_position([0.3,0.02,0.9,0.9])              #Se ubica posición y tamaño
pi_symb = unicodedata.lookup("GREEK SMALL LETTER PI")   #se crea el símbolo pi
max_min = unicodedata.lookup("plus-minus sign")         #se crea el símbolo +-
...
#Se toman las barras creadas y se convierten en sliders y botones para reset y simulación 
s_tP   = Slider(ax_tP, 'P',     -1, 1, valinit=(theta_vals[0]/(m.pi)), valstep=0.01, valfmt = "%5.2f"+pi_symb)
s_tQ   = Slider(ax_tQ, 'Q',     -1, 1, valinit=(theta_vals[1]/(m.pi)), valstep=0.01, valfmt = "%5.2f"+pi_symb)
s_tR   = Slider(ax_tR, 'R',     -1, 1, valinit=(theta_vals[2]/(m.pi)), valstep=0.01, valfmt = "%5.2f"+pi_symb)
s_tS   = Slider(ax_tS, 'S',     -1, 1, valinit=(theta_vals[3]/(m.pi)), valstep=0.01, valfmt = "%5.2f"+pi_symb)
s_tTd  = Slider(ax_tTd, 'Td',   -1, 1, valinit=(theta_vals[4]/(m.pi)), valstep=0.01, valfmt = "%5.2f"+pi_symb)
s_tTu  = Slider(ax_tTu, 'Tu',   -1, 1, valinit=(theta_vals[5]/(m.pi)), valstep=0.01, valfmt = "%5.2f"+pi_symb)
...
#Se crean las visualizaciónes de las posiciones angulares para cada atractor correspondiente a cada onda. 
#Se ubican en sus posiciones default. 
p_P  = ax_circle.scatter(np.cos(theta_vals[0]), np.sin(theta_vals[0]))
p_Q  = ax_circle.scatter(np.cos(theta_vals[1]), np.sin(theta_vals[1]))
p_R  = ax_circle.scatter(np.cos(theta_vals[2]), np.sin(theta_vals[2]))
p_S  = ax_circle.scatter(np.cos(theta_vals[3]), np.sin(theta_vals[3]))
p_Td = ax_circle.scatter(np.cos(theta_vals[4]), np.sin(theta_vals[4]))
p_Tu = ax_circle.scatter(np.cos(theta_vals[5]), np.sin(theta_vals[5]))

#Se crean los títulos de cada atractor (P, Q, R, S, Td, Tu)
an_P  = ax_circle.annotate('P', (1.1*np.cos(theta_vals[0]), 1.1*np.sin(theta_vals[0])))
an_Q  = ax_circle.annotate('Q', (1.1*np.cos(theta_vals[1]), 1.1*np.sin(theta_vals[1])))
an_R  = ax_circle.annotate('R', (1.1*np.cos(theta_vals[2]), 1.1*np.sin(theta_vals[2])))
an_S  = ax_circle.annotate('S', (1.1*np.cos(theta_vals[3]), 1.1*np.sin(theta_vals[3])))
an_Td = ax_circle.annotate('Td', (1.1*np.cos(theta_vals[4]), 1.1*np.sin(theta_vals[4])))
an_Tu = ax_circle.annotate('Tu', (1.1*np.cos(theta_vals[5]), 1.1*np.sin(theta_vals[5])))


#Se crean los 'ticks' para el eje angular. -> Se crean como rayos radiales dibujados a una distancia [inte - exte] desde el centro. 
#Además se crean los valores para estos ticks con .annotate()
exte = 1.01
inte = 0.99
offset = [0.08, 0.04]
...
#Esta función permite que los puntos de visualización se muevan a la par que el slider correspondiente en tiempo real
#(lo que es imposible para la generación de datos, porque implica demasiado procesamiento imposible de completar en tiempo real. Por esta razón se usa el botón 'simular')
def update_circle(val):
    #Se calculan los nuevos ángulos para cada atractor. (de acuerdo a modelo en paper)
    ...
    #Se reubican los puntos de visualización en el círculo. 
    ...
    #Se reubican los título de los puntos de visualización en el círculo. 
...
#Permite reactividad en tiempo real de esta visualización de acuerdo a todos los parámetros involucrados. 
#Igual que antes, la reactividad interrelacionada en tiempo real para todos los parámetros involucrados es posible
#Porque la visualización necesita muy poco procesamiento. Lo que no es posible para la generación de la señal. 
...
#Creación los plot para la visualización de las ondas componentes. 
ax_gauss = fig_gauss.subplots(1,6, sharex = True, sharey = True) #El sharex/sharey permite que las visualizaciones cambien según los tamaños relativos. 
fig_gauss.subplots_adjust(0.05,0.625, 0.95, 0.825)

#Identificación de cada plot. 
ondas = ('P','Q','R','S','Td','Tu')
for i in range(len(ondas)) :
    ax_gauss[i].set_title(ondas[i])

#Función para la Construcción de la señal gaussiana para su ploteo. 
#Con las adaptaciones consideradas según el paper. 
...
#Construcción de la gaussiana para cada onda según valores default. 
sig_P =     gaussian(a_vals[0], b_vals[0])
sig_Q =     gaussian(a_vals[1], b_vals[1])
sig_R =     gaussian(a_vals[2], b_vals[2])
sig_S =     gaussian(a_vals[3], b_vals[3])
sig_Td =    gaussian(a_vals[4], b_vals[4])
sig_Tu =    gaussian(a_vals[5], b_vals[5])

#Ploteo inicial de cada gaussiana. 
#Se retorna el axis para poder actualizar sus datos 
...
#Sobre estos elementos se construye los slider y los botones de reset y simulación
width_g = 0.045
#Barras para el valor a (amplitud)
...
#Barras para el valor b (duración)
...
def update_gauss(val):
    ...
    #Se crean las nuevas señales para las gaussianas
    ...
    #Se actualiza los ploteos con las señales recién creadas. 
...
#Igual que antes, ante algún cambio en el slider, se activa la función de actualización de la señal
